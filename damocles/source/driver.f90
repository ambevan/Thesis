MODULE driver
    ...
    INTEGER ::  lgabs,lgactive,idP_thread,test
    INTEGER ::  iG,iG_axis(3),freqid,los
    ...
    SUBROUTINE run_code(param_struct,chi2)
        ...
        DO iDoublet=1,2
            ...
            ELSE
                ...
                DO iSh=1,n_shells														!divide the SN up into radial shells
...
!                        PRINT*,'threadno',omp_get_thread_num()
        ...
        IF (lgDoublet) THEN
            E_0=L_tot/(2*n-n_inactive)												!Energy of a single packet in W/um (note uses actual number of active photons)
        ELSE
            E_0=L_tot/(n-n_inactive)
        END IF
        ...
        IF (lgDoublet) THEN
           PRINT*,'undepleted luminosity (in units e40 erg/s)',L_tot/(1-w_abs/real(2*n-n_inactive))
        ELSE
