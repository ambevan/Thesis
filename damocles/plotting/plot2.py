...
wave,vel1,flux_mod=np.loadtxt('output/output.out',skiprows=1,usecols=(0,1,2),unpack=True)
...
flux_out_dat=flux_out_dat-np.nanmin(flux_out_dat)-1.0e-16
...
flux2_dat=flux2_dat-np.nanmin(flux2_dat)-1.0e-16
flux_mod=flux_mod*s*(np.nanmax(flux2_dat))/np.nanmax(flux_mod)
...
line_2=ax1.plot(vel1,flux_mod,'r',label='mod')
