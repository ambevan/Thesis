...
infile1 = "./runA2/test_output_chain_1"
...
for text_line in chain_text:
	...
	dens.append(3.*float(text_list[2]) + 0. )
	temp.append(2.*float(text_list[3]) + 1. )
	av.append(2. * float(text_list[4]))
	g0.append(5. * float(text_list[5]) - 2. )
...
image = axarr[1,0].hexbin(dens,zeta)
image = axarr[1,2].hexbin(dens,drift)
...
sample = [sample_dens,sample_temp,sample_av,sample_g0,sample_zeta,sample_drift]
